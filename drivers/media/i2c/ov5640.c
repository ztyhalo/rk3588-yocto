...
static s32 hn_ov5640_read_reg(struct ov5640_dev *sensor, u16 reg, u8 *val)
{
	u8 au8RegBuf[2] = {0};
	u8 u8RdVal = 0;
					 ...
					 const struct ov5640_mode_info *mode)
{
	...
	printk("hndz ov5640_set_mode_exposure_calc!\n");
	ret = ov5640_load_regs(sensor, mode);
	if (ret < 0)
		return ret;
	printk("hndz ov5640_set_mode_exposure_calc end!\n");
	/* read capture VTS */
	ret = ov5640_get_vts(sensor);
	if (ret < 0)
		return ret;
				  ...
				  const struct ov5640_mode_info *mode)
{
	...
	/* Write capture setting */
	printk("hndz ov5640_set_mode_direct!\n");
...
static int ov5640_restore_mode(struct ov5640_dev *sensor)
{
	...
	if (ret < 0)
		return ret;
	printk("hndz ov5640_restore_mode end!\n");
...
static void ov5640_reset(struct ov5640_dev *sensor)
{
	if (!sensor->reset_gpio)
		return;
	printk("hndz ov5640 reset start!\n");
	// dump_stack();
...
static int ov5640_set_power(struct ov5640_dev *sensor, bool on)
{
	...
	if (sensor->ep.bus_type == V4L2_MBUS_CSI2_DPHY)
	{
		printk("hndz ov5640 type V4L2_MBUS_CSI2_DPHY!\n");
		ret = ov5640_set_power_mipi(sensor, on);
	}
	else
	{
		printk("hndz ov5640 type dvp!\n");
...
{
	printk("hndz ov5640 enum frame size!\n");
	if (fse->pad != 0)
		return -EINVAL;
...
static int ov5640_check_chip_id(struct ov5640_dev *sensor)
{
	...
	// retval = hn_ov5640_read_reg(sensor, OV5640_CHIP_ID_HIGH_BYTE, &chip_id_high);
	// if (retval < 0 || chip_id_high != 0x56) {
	// 	printk("camera ov5640_mipi is not found\n");

	// 	return -ENXIO;
	// }
	// retval = hn_ov5640_read_reg(sensor, OV5640_CHIP_ID_LOW_BYTE, &chip_id_low);
...
static int ov5640_probe(struct i2c_client *client)
{
	...
	// if(sensor->pwdn_gpio->name != NULL)
	// 	printk("hndz pwdn gpio name %s!\n", sensor->pwdn_gpio->name);
	/* request optional reset pin */
	sensor->reset_gpio = devm_gpiod_get_optional(dev, "reset",
						     GPIOD_OUT_HIGH);
	if (IS_ERR(sensor->reset_gpio))
		return PTR_ERR(sensor->reset_gpio);
	// if(sensor->reset_gpio->name != NULL)
	// 	printk("hndz reset_gpio gpio name %s!\n", sensor->reset_gpio->name);
	printk("hndz old ov5640!\n");
