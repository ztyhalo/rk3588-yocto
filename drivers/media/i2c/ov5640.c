...
{
		printk("hndz ov5640is  csi2 type %d", sensor->ep.bus_type);
...
static const struct reg_value ov5640_init_setting[] = {
	{0x3103, 0x11, 0, 0},
	{0x302c, 0x82, 0, 0},
	{0x3017, 0x00, 0, 0}, {0x3018, 0x00, 0, 0},
...
{
	printk("hndz get ov5640 timings!\n");
					    ...
					    u8 sysdiv)
{
	...
	if (sysclk / 1000000 > 1000)
		return 0;
	printk("hndz sysclk %d sysdiv %d val %d!\n", sysclk, sysdiv, sysclk / sysdiv);
					 ...
					 u8 *sysdiv)
{
	...
	printk("hndz calc sys clk rate %ld!\n", rate);
...
out:
	...
	*pll_mult = best_mult;
	printk("hndz cala sys clk best_sysdiv %d OV5640_PLL_PREDIV %d pll_mult %d best %d!\n", best_sysdiv,OV5640_PLL_PREDIV,best_mult, best);
	return best;
}
...
static int ov5640_set_mipi_pclk(struct ov5640_dev *sensor)
{
	...
	printk("hndz set mipi pclk link_freq %ld sysclk %ld!\n", link_freq, sysclk);
...
static u32 ov5640_calc_pixel_rate(struct ov5640_dev *sensor)
{
	...
	printk("hndz ov5640 calc pixel rate %d!\n", rate);
				      ...
				      u8 *pll_rdiv, u8 *bit_div, u8 *pclk_div)
{
	...
	*pclk_div = OV5640_PCLK_ROOT_DIV;
	printk("hndz ov5640_calc_pclk %ld!\n", _rate / *pll_rdiv / *bit_div / *pclk_div);
			      ...
			      const struct ov5640_mode_info *mode)
{
	...
	printk("hndz set timings ensor->fmt.code 0x%x!\n", sensor->fmt.code);
...
static int ov5640_get_exposure(struct ov5640_dev *sensor)
{
	...
	if (ret)
		return ret;
	exp |= (int)temp;
	printk("hndz read exposure 0x%x!\n", exp);
	return exp >> 4;
}
...
static int ov5640_get_gain(struct ov5640_dev *sensor)
{
	...
	if (ret)
		return ret;
	printk("hndz get gain 0x%d!\n", gain);
	return gain & 0x3ff;
}

static int ov5640_set_gain(struct ov5640_dev *sensor, int gain)
{
	printk("hndz set gain 0x%x!\n", gain);
...
static int ov5640_set_stream_mipi(struct ov5640_dev *sensor, bool on)
{
	 ...
	 */
	printk("hndz set stream mipi on %d!\n", on);
...
static int ov5640_get_vts(struct ov5640_dev *sensor)
{
	...
	if (ret)
		return ret;
	printk("hndz get vts %d!\n", vts);
	return vts;
}

static int ov5640_set_vts(struct ov5640_dev *sensor, int vts)
{
	printk("hndz set vts %d!\n", vts);
...
static int ov5640_set_virtual_channel(struct ov5640_dev *sensor)
{
	...
	temp &= ~(3 << 6);
	temp |= (channel << 6);
	printk("hndz set virtual channel %d!\n", temp);
					 ...
					 const struct ov5640_mode_info *mode)
{
	...
	if (!mode->reg_data)
		return -EINVAL;
	printk("hndz set mode exposure calc mode id %d!\n", mode->id);
				  ...
				  const struct ov5640_mode_info *mode)
{
	if (!mode->reg_data)
		return -EINVAL;
	printk("hndz set mode direct mode id %d!\n", mode->id);
	/* Write capture setting */
	ov5640_load_regs(sensor, mode->reg_data, mode->reg_data_size);
...
static int ov5640_set_mode(struct ov5640_dev *sensor)
{
	...
	printk("hndz set mode dnmoe %d orign mode %d auto gain %d exp %d!\n", dn_mode, orig_dn_mode, auto_gain, auto_exp);
...
static int ov5640_restore_mode(struct ov5640_dev *sensor)
{
	...
	printk("hndz ov5640_restore_mode!\n");
	ret = ov5640_mod_reg(sensor, OV5640_REG_SYS_ROOT_DIVIDER, 0x3f,
...
static int ov5640_set_power_on(struct ov5640_dev *sensor)
{
	struct i2c_client *client = sensor->i2c_client;
	int ret;
	printk("hndz ov5640_set_power_on start!\n");
	// dump_stack();
...
static int ov5640_sensor_suspend(struct device *dev)
{
	...
	printk("hndz ov5640_sensor_suspend!\n");
...
static int ov5640_sensor_resume(struct device *dev)
{
	struct v4l2_subdev *sd = dev_get_drvdata(dev);
	struct ov5640_dev *ov5640 = to_ov5640_dev(sd);
	printk("hndz ov5640_sensor_resume!\n");
				     ...
				     u32 width, u32 height)
{
	...
	printk("hndz ov5640_try_frame_interval best_fps %d!\n", best_fps);
			  ...
			  struct v4l2_subdev_format *format)
{
	...
	struct v4l2_mbus_framefmt *fmt;
	printk("hndz ov5640 get fmt start!\n");
	// dump_stack();
	if (format->pad != 0)
		return -EINVAL;
				   ...
				   const struct ov5640_mode_info **new_mode)
{
	...
	unsigned int bpp;
	printk("hndz ov5640_try_fmt_internal!\n");
	...
	if (new_mode)
		*new_mode = mode;
	printk("hndz ov5640_try_fmt_internal pixfmt->code 0x%x!\n", pixfmt->code);
...
static void __v4l2_ctrl_vblank_update(struct ov5640_dev *sensor, u32 vblank)
{
	...
	printk("hndz ctrl vblank update %d!\n", vblank);
...
static int ov5640_update_pixel_rate(struct ov5640_dev *sensor)
{
	...
	printk("hndz sensor->current_link_freq %ld!\n", sensor->current_link_freq);
	/*
			  ...
			  struct v4l2_subdev_format *format)
{
	...
	int ret;
	printk("hndz ov5640 ov5640_set_fmt fmt start!\n");
	// dump_stack();
	if (format->pad != 0)
		return -EINVAL;
...
// 	const struct ov5640_mode_info *mode = sensor->current_mode;
// 	const struct ov5640_timings *timings;
// 	printk("hndz get selection start!\n");
// 	dump_stack();
// 	switch (sel->target) {
			       ...
			       struct v4l2_mbus_framefmt *format)
{
	...
	const struct ov5640_pixfmt *pixfmt;
	int ret = 0;
	printk("hndz set frame fmt!\n");
	// dump_stack();
	pixfmt = ov5640_code_to_pixfmt(sensor, format->code);
	printk("hndz pixfmt 0x%x!\n", pixfmt->code);
...
static int ov5640_set_ctrl_hue(struct ov5640_dev *sensor, int value)
{
	...
	printk("hndz ov5640_set_ctrl_hue value %d!\n", value);
	return ret;
}
...
{
	int ret;
	printk("hndz ov5640_set_ctrl_contrast value %d!\n", value);
	if (value) {
		ret = ov5640_mod_reg(sensor, OV5640_REG_SDE_CTRL0,
				     BIT(2), BIT(2));
		if (ret)
...
{
	int ret;
	printk("hndz ov5640_set_ctrl_saturation value %d!\n", value);
	if (value) {
		ret = ov5640_mod_reg(sensor, OV5640_REG_SDE_CTRL0,
				     BIT(1), BIT(1));
		if (ret)
...
{
	int ret;
	printk("hndz ov5640_set_ctrl_white_balance value %d!\n", awb);
				    ...
				    enum v4l2_exposure_auto_type auto_exposure)
{
	...
	int ret = 0;
	printk("hndz ov5640_set_ctrl_exposure value %d!\n", auto_exposure);
...
static int ov5640_set_ctrl_gain(struct ov5640_dev *sensor, bool auto_gain)
{
	struct ov5640_ctrls *ctrls = &sensor->ctrls;
	int ret = 0;
	printk("hndz ov5640_set_ctrl_gain value %d!\n", auto_gain);
...
{
	printk("hndz set ctrl test pattern value %d!\n", value);
...
{
	int ret;
	printk("hndz ov5640_set_ctrl_light_freq value %d!\n", value);
...
static int ov5640_set_ctrl_hflip(struct ov5640_dev *sensor, int value)
{
	 ...
	 * - [1]:	Sensor mirror
	 */
	printk("hndz ov5640_set_ctrl_hflip value %d!\n", value);
...
static int ov5640_set_ctrl_vflip(struct ov5640_dev *sensor, int value)
{
	 ...
	 * - [1]:	Sensor vflip
	 */
	printk("hndz ov5640_set_ctrl_vflip value %d!\n", value);
	return ov5640_mod_reg(sensor, OV5640_REG_TIMING_TC_REG20,
			      BIT(2) | BIT(1),
			      (value ^ sensor->upside_down) ?
			      (BIT(2) | BIT(1)) : 0);
...
static int ov5640_set_ctrl_vblank(struct ov5640_dev *sensor, int value)
{
	...
	printk("hndz ov5640_set_ctrl_vblank value %d!\n", value);
...
static int ov5640_g_volatile_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	if (!pm_runtime_get_if_in_use(&sensor->i2c_client->dev))
		return 0;
	printk("hndz ov5640_g_volatile_ctrl ctrl id 0x%x!\n", ctrl->id);
	switch (ctrl->id) {
...
static int ov5640_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	/* v4l2_ctrl_lock() locks our own mutex */
	printk("hndz ov5640_s_ctrl ctrl id 0x%x!\n", ctrl->id);
	switch (ctrl->id) {
				  ...
				  struct v4l2_subdev_frame_size_enum *fse)
{
	...
	printk("hndz enum frame size bpp %d index %d!\n", bpp, index);
	if (fse->pad != 0)
		return -EINVAL;
	if (!bpp)
		return -EINVAL;
	...
	struct v4l2_subdev_frame_interval_enum *fie)
{
	...
	if (ret < 0)
		return -EINVAL;
	printk("hndz ov5640_enum_frame_interval fie width %d fie->index%d!\n", fie->width, fie->index);
	fie->interval = tpf;
	return 0;
}
...
{
	struct ov5640_dev *sensor = to_ov5640_dev(sd);
	printk("hndz ov5640_g_frame_interval!\n");
	mutex_lock(&sensor->lock);
				   ...
				   struct v4l2_subdev_frame_interval *fi)
{
	...
	if (fi->pad != 0)
		return -EINVAL;
	printk("hndz ov5640_s_frame_interval!\n");
	mutex_lock(&sensor->lock);
				 ...
				 struct v4l2_subdev_mbus_code_enum *code)
{
	...
	printk("hndz ov5640_enum_mbus_code!\n");
	if (ov5640_is_csi2(sensor)) {
...
static int ov5640_s_stream(struct v4l2_subdev *sd, int enable)
{
	struct ov5640_dev *sensor = to_ov5640_dev(sd);
	int ret = 0;
	printk("hndz ov5640_s_stream enable %d!\n", enable);
	if (enable) {
...
static long ov5640_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	struct ov5640_dev *sensor = to_ov5640_dev(sd);
	long ret = 0;
	printk("hndz ov5640 ioctl cmd 0x%x!\n", cmd);
	switch (cmd) {
...
{
		u32 val = 0;
	printk("hndz ov5640_g_mbus_config!\n");
