...
ov5640_code_to_pixfmt(struct ov5640_dev *sensor, u32 code)
{
	...
	for (i = 0; formats[i].code; ++i) {
		...
		{
			printk("hndz find code 0x%x!\n", code);
			return &formats[i];
		}
	}
	printk("hndz no find code 0x%x!\n", code);
	return &formats[0];
}
...
static const struct reg_value ov5640_setting_low_res[] = {
	{0x3035, 0x14, 0, 0}, {0x3036, 0x60, 0, 0}, {0x3c07, 0x08, 0, 0},
	{0x3c09, 0x1c, 0, 0}, {0x3c0a, 0x9c, 0, 0}, {0x3c0b, 0x40, 0, 0},
	{0x3820, 0x41, 0, 0}, {0x3821, 0x01, 0, 0}, {0x3814, 0x31, 0, 0},
	{0x3815, 0x31, 0, 0}, {0x3800, 0x00, 0, 0}, {0x3801, 0x00, 0, 0},
	{0x3802, 0x00, 0, 0}, {0x3803, 0x04, 0, 0}, {0x3804, 0x0a, 0, 0},
			      ...
			      const struct ov5640_mode_info *mode)
{
	...
	if (ret < 0)
		return ret;
	printk("hndz wirte timed htot %d!\n", timings->htot);
					 ...
					 const struct ov5640_mode_info *mode)
{
	...
	if (!mode->reg_data)
		return -EINVAL;
	printk("hndz set mode exposure calc ov5640_mode_id %d!\n", mode->id);
...
static int ov5640_set_mode(struct ov5640_dev *sensor)
{
	...
	if (ret < 0)
		return 0;
	printk("hndz dn mode is %d orig dn mode is %d!\n", dn_mode, orig_dn_mode);
				   ...
				   const struct ov5640_mode_info **new_mode)
{
	...
	if (!mode)
		return -EINVAL;
	printk("hndz try fmt internal mode id %d!\n", mode->id);
...
static int ov5640_update_pixel_rate(struct ov5640_dev *sensor)
{
	 ...
	 */
	printk("hndz ov5640_update_pixel_rate start!\n");
	if (!ov5640_is_csi2(sensor)) {
	...
	printk("hndz ov5640_update_pixel_rate num_lanes %d bpp %d!\n", num_lanes, bpp);
	do {
	...
	printk("hndz ov5640_update_pixel_rate link freq %d!\n", link_freq);
	/*
	...
	printk("hndz find link freqs i %d!\n", i);
				...
				struct v4l2_subdev_selection *sel)
{
	...
	const struct ov5640_mode_info *mode = sensor->current_mode;
	const struct ov5640_timings *timings;
	printk("hndz get selection start sel->target 0x%x!\n", sel->target);
	// dump_stack();
	switch (sel->target) {
			       ...
			       struct v4l2_mbus_framefmt *format)
{
	...
	const struct ov5640_pixfmt *pixfmt;
	int ret = 0;
	printk("hndz set frame fmt code 0x%x!\n", format->code);
	// dump_stack();
...
static int ov5640_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	printk("hndz func %s line %d ctrl->id %d!\n", __func__, __LINE__, ctrl->id);
	/* v4l2_ctrl_lock() locks our own mutex */
			...
			const struct i2c_device_id *did)
{
	...
	printk("hndz parse ep bus type 0x%x!\n", sensor->ep.bus_type);
